...
dat <- with(pkgs, data.frame(
  Name        = sprintf(
    "[**%s**](%s)", name, ifelse(!is.na(repo), repo, paste0("https://github.com/USCbiostats/", name))
    ),
  Description = paste(
    description,
    sprintf(
...
```{r citations, include=TRUE, warning=TRUE, message=TRUE}
library(RSelenium)

# Setting up the server
ps <- system("docker run -d -p 4445:4444 selenium/standalone-firefox:2.53.1")
remDr <- remoteDriver(
...
for (i in seq_len(nrow(pkgs))) {
  ...
  results <- remDr$findElements(using = "class", "gs_ab_mdw")
  results <- sapply(results, function(r) r$getElementText())
  results <- results[which(grepl(".+About\\s*([0-9,]+)\\*results", results))]
